...
import com.example.demo.CartAnalyzer;
...
public class CartService {
    ...
    public CartEntity createCart(CartEntity cartEntity, UUID productId) {
        ...
        double productCost = cartAnalyzer.calculateCartItemProductCost(temp.get().getProductPrice(), cartEntity.getSelectedProductKol());
    ...
    public Optional<CartEntity> modifyItem(UUID id, CartEntity cartEntity) {
        ...
        return result
                .map(entity -> {
                    ...
                    double productCost = cartAnalyzer.calculateCartItemProductCost(cartEntity.getCatalogProductPrice(), cartEntity.getSelectedProductKol());
    ...
    public Page<CartEntity> getAllCart(String filter, Pageable pageable) {
        if (filter != null && !filter.isBlank()) {
            ...
            cartAnalyzer.calculateFinalCartKol(cartRepository.findAll(pageable));
            cartAnalyzer.calculateFinalCartPrice(cartRepository.findAll(pageable));
            cartAnalyzer.checkDiscount();
